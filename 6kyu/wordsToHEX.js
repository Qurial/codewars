...
function wordsToHex(words) {
  const charToAscii = (word, index) => {
    return word.charAt(index) ? word.charAt(index).charCodeAt(0).toString(16) : "00"
  }
  let hexArr = []
  const wordsArr = words.split(" ");
  for(let word of wordsArr){
    hexArr.push("#" + charToAscii(word, 0) + charToAscii(word, 1) + charToAscii(word, 2))
  }
  return hexArr
}
